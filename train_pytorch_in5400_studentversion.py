...
def train_epoch_twoNets(model, trainloader, criterion, device, optimizer):
...
def evaluate_meanavgprecision_twoNets(model, dataloader, criterion, device, numcl):
...
def traineval2_model_nocv_twoNets(dataloader_train, dataloader_test ,  model ,  criterion, optimizer, scheduler, num_epochs, device, numcl):
  ...
  for epoch in range(num_epochs):
    ...
    avgloss, train_mAP = train_epoch_twoNets(model,  dataloader_train,  criterion,  device , optimizer )
    ...
    perfmeasure, testloss,concat_labels, concat_pred, fnames  = evaluate_meanavgprecision_twoNets(model, dataloader_test, criterion, device, numcl)
...
def runstuff3(): # Two nets, 4 channels kaiminghe
  config = dict()
  config['use_gpu'] = True #True #TODO change this to True for training on the cluster
  config['lr'] = 0.01
  config['batchsize_train'] = 16
  ...
  # Model
  # TODO create an instance of the network that you want to use.
  pretrained_net1 = resnet18(pretrained=True)# TwoNetworks()
  pretrained_net2 = resnet18(pretrained=True)
  ...
  best_epoch, best_measure, bestweights, trainlosses, testlosses, testperfs, trainperfs, preds, labels, fnames = traineval2_model_nocv_twoNets(dataloaders['train'], dataloaders['val'] ,  model ,  lossfct, someoptimizer, somelr_scheduler, num_epochs= config['maxnumepochs'], device = device , numcl = config['numcl'] )
  print("Best epoch: ", best_epoch)
  print("Best measure: ", best_measure)
  # Save best weights
  torch.save(bestweights, "./best_model_two.pt")
  # Write important outputs to file
  file_classes = open('classes_avg_scores_two.csv', 'w')
  class_writer = csv.writer(file_classes)
  header_row_classes = ['clear', 'cloudy', 'haze', 'partly_cloudy',
             'agriculture', 'artisinal_mine', 'bare_ground', 'blooming',
             'blow_down', 'conventional_mine', 'cultivation', 'habitation',
             'primary', 'road', 'selective_logging', 'slash_burn', 'water']
  class_writer.writerow(header_row_classes)
  file_losses = open('losses_mAP_two.csv', 'w')
  loss_writer = csv.writer(file_losses)
  header_row_losses = ['training_loss','validation_loss','mAP_test', 'mAP_train']
  loss_writer.writerow(header_row_losses)
  ...
  file_classes.close()
  file_losses.close()
  file_fnames = open('fnames_two.csv', 'w')
  name_writer = csv.writer(file_fnames)
  name_writer.writerow(["filenames"])


  file_pred = open('predictions_two.csv', 'w')
  pred_writer = csv.writer(file_pred)
  pred_writer.writerow(header_row_classes)

  file_lbls = open('labels_two.csv', 'w')
  lbls_writer = csv.writer(file_lbls)
  lbls_writer.writerow(header_row_classes)
...
def runstuff4(): # Single net, 4 channels kaiming-he
  config = dict()
  config['use_gpu'] = True #True #TODO change this to True for training on the cluster
  config['lr'] = 0.01
  config['batchsize_train'] = 16
  ...
  # TODO create an instance of the network that you want to use.
  pretrained_net = resnet18(pretrained=True)# TwoNetworks()
  model = SingleNetwork(pretrained_net, weight_init='kaiming-he')
